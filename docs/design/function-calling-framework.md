...
## ⚠️ 项目范围说明（2026-01-16 更新）

**实际实施范围**：
- ✅ **核心框架**：ToolRegistry + ToolExecutor（已实现）
- ✅ **文本处理工具**：文章总结、关键信息提取（已实现）
- ✅ **天气查询工具**：wttr.in API（已实现）
- ❌ **不再开发**：价格对比、航班查询、任务管理、图片处理等工具

**调整理由**：
- 保持项目轻量级定位
- 避免引入复杂的第三方 API 依赖
- 降低维护成本
- 当前工具已充分验证 Function Calling 框架的可行性
...
### 3.2 Function Calling 核心框架 🔴 P0

**功能边界**：
- **实施阶段**：阶段 1.A（必需，10.5 小时）
- **优先级**：🔴 P0（核心框架）
- **依赖**：无依赖，可独立实施
...
**功能边界**：
- **实施阶段**：阶段 2（必需，0.5-1 天）
- **优先级**：🟡 P1（高优先级，但不是 P0）
- **依赖**：3.2 核心框架（ToolExecutor）
- **输出**：GuidanceHandler + 参数验证逻辑
- **验收**：参数不完整时 AI 主动询问
- **复杂功能**：🔀 建议创建独立文档 `smart-guidance-system.md` 后实施
- **特点**：扩展核心框架，不修改基础逻辑
...
#### 3.4.2 用户上下文系统（个性化支持） 🟡 P1

**功能边界**：
- **实施阶段**：阶段 2.5（可选，4.5 小时）
- **优先级**：🟡 P1（高优先级，但不是 P0）
- **依赖**：3.2 核心框架 + 前端（localStorage）
- **输出**：UserContextManager（客户端）+ 工具参数增强
- **验收**：用户设置个性化后，工具返回个性化结果
- **复杂功能**：🔀 建议创建独立文档 `user-context-system.md` 后实施
- **特点**：独立功能，使用客户端 localStorage，无服务器状态
