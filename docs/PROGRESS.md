...
- [x] **阶段 1.C：基础错误处理**（2026-01-15）
  - ✅ `tools/utils/error-handler.js`
  - ✅ AppError 类及错误代码枚举
  - ✅ 错误日志记录
...
- [x] **阶段 2：智能引导**（2026-01-15）✨
  - ✅ `tools/utils/param-validator.js` - 参数验证模块
  - ✅ 意图识别（关键词匹配）
  - ✅ 参数完整性检查
  - ✅ 引导提示生成
  - ✅ 集成到聊天接口

- [x] **阶段 2.5：用户上下文系统**（2026-01-15）✨
...
```javascript
tools/
├── index.js                 # ToolRegistry + ToolExecutor (155 行)
├── text-tools.js            # 文本处理工具（2个）(181 行)
└── utils/
    ├── error-handler.js     # 错误处理模块 (138 行)
    └── param-validator.js   # 参数验证和智能引导 (240 行)

API 接口：
├── GET /api/tools           # 获取工具列表 ✅
└── POST /api/chat/tools     # 智能聊天接口 ✅（含引导）
```
...
**用户上下文系统**：
- ✅ localStorage 存储（`chatbot_user_context`）
- ✅ 快速开始使用统计
- ✅ 工具使用统计
- ✅ 偏好功能排序
- ✅ 自动记录用户行为
...
| **阶段3** | **Function Calling 框架** | ⭐⭐⭐⭐⭐ | 🔧🔧🔧🔧 | 🔴 P0 | 3-5天 | 🚧 进行中 | 架构升级（60%） |
| 阶段3 | ToolRegistry + ToolExecutor | ⭐⭐⭐⭐⭐ | 🔧🔧🔧🔧 | 🔴 P0 | 1-2天 | ✅ 已完成 | 核心框架 ✅ |
| 阶段3 | **基础错误处理** | ⭐⭐⭐⭐⭐ | 🔧🔧 | 🔴 P0 | 2h | ✅ 已完成 | AppError + ErrorHandler ✅ |
| 阶段3 | **快速开始（Aha Moment）** | ⭐⭐⭐⭐⭐ | 🔧🔧 | 🔴 P0 | 4.5h | ✅ 已完成 | 新用户体验 ✅ |
| 阶段3 | **用户上下文系统** | ⭐⭐⭐⭐ | 🔧🔧 | 🟡 P1 | 4.5h | ✅ 已完成 | 客户端 localStorage ✅ |
| 阶段3 | 文本处理工具 | ⭐⭐⭐⭐ | 🔧🔧 | 🟡 P1 | 1天 | ✅ 已完成 | 文章总结等 ✅ |
| 阶段3 | 智能引导 | ⭐⭐⭐⭐ | 🔧🔧 | 🟡 P1 | 0.5-1天 | ✅ 已完成 | 参数补全 ✅ |
| **阶段4** | **天气查询** | ⭐⭐⭐⭐ | 🔧🔧 | 🟡 P1 | 2天 | 📋 待调研 | 外部API |
| 阶段4 | **商品价格对比** | ⭐⭐⭐⭐ | 🔧🔧🔧 | 🟡 P1 | 3天 | 📋 待调研 | 外部API |
| 阶段4 | **任务管理** | ⭐⭐⭐⭐⭐ | 🔧🔧🔧🔧 | 🟡 P1 | 5天 | 📋 延后 | 需存储升级 |
| 阶段4 | 航班查询 | ⭐⭐⭐ | 🔧🔧🔧 | 🟢 P2 | 3天 | 📋 待调研 | 外部API |
| 阶段4 | 图片去水印 | ⭐⭐⭐ | 🔧🔧 | 🟢 P2 | 2天 | 📋 待调研 | 文件处理 |
| 阶段4 | 历史统计看板 | ⭐⭐⭐ | 🔧🔧🔧 | 🟢 P2 | 3天 | 📋 延后 | 需存储升级 |

**说明**：
- 🔴 P0：必须立即完成（阶段0代码结构 + 阶段3核心框架 + 基础错误处理 + 用户体验）✅ 已全部完成
- 🟡 P1：高优先级（阶段4核心功能 + 用户上下文系统）✅ 用户上下文已完成
- 🟢 P2：中优先级（增强功能）
- "待调研"：需要先调研外部API
- "延后"：需要存储架构升级后再实施
...
**说明**：阶段1和阶段2的详细Week计划已完成，此处省略。当前聚焦阶段3和阶段4。

### 阶段3：Function Calling 框架（进行中）
...
**待完成**：
- [ ] 完整 Function Calling（AI API 原生支持）
- [ ] 多轮工具调用
- [ ] 工具结果整合优化
- [ ] 性能优化和测试
...
1. **完整 Function Calling 未实现**（优先级：中）
   - 影响：当前使用简化的关键词匹配，非 AI 原生工具调用
   - 计划：后续升级到 AI API 原生 Function Calling 支持
...
**开始时间**：2026-01-12
**当前版本**：v2.1.0（阶段 3 完成 60%）
**下一版本**：v2.2.0（外部 API 集成）
**下一版本**：v3.0.0（智能助理功能）
**预计完成**：1-2周（阶段 3 进行中）
