...
- [x] 创建 Node.js 后端服务（`server.js`，297 行）
  - GLM-4 API 集成
...
```javascript
// 新增功能
├── API 配置（API_BASE_URL、currentProvider）
├── 对话历史管理
│   ├── conversationHistory
│   ├── saveConversationHistory()
│   └── loadConversationHistory()
└── API 调用（sendMessage() 改为异步）
```
...
**后端任务**：
- [ ] 创建 `/api/chat/stream` 接口
- [ ] GLM API 流式调用
- [ ] DeepSeek API 流式调用
- [ ] 错误处理（流中断）

**前端任务**：
- [ ] 修改 `sendMessage()` 使用流式接口
- [ ] 实现打字机动画效果
- [ ] 处理流中断和错误
- [ ] 测试不同长度的回复
